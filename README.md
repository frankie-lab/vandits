...
- ✅ Tests nuevos: `territorial-canon-wire-resolver-pt-insular` (5 casos, harness simétrico al wire-resolver base) + `territorial-canon-wire-resolver-edge-contract` (paridad runtime PT-20/PT-30/PT-11 TS↔Deno + wiring de edge e cliente). Lint anti-hardcode extiende `GUARDED_FILES` con `supabase/functions/resolve-admin-area/index.ts`.
- ✅ Cierra criterios §5.2 (edge enforcement) y §5.3 (rechazos server-side) del roadmap territorial — paso de 2/6 a 4/6.
- ✅ Bump **patch** `1.3.16 → 1.3.17`. NO toca: datos (`locations`, `admin_areas`, `enriched_data`), Santa Cruz da Graciosa, Bolhão, Braga Parque, Nominatim, re-enrich, migraciones SQL, renderer del mapa, tokens POI-N, health rings, colecciones, RLS.
