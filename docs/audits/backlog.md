...
| BL-021 | Fase B RBAC pendiente — purga destructiva del enum `public.app_role` (drop de `'curator'` y `'user'`; decisión `'supervisor'`). Bloqueado por validación post-Fase A. | rbac-canon | low | Catálogo enum aún expone valores zombi (0 titulares, 0 caps) | identity / db | open | mem://governance/rbac-canon |
