...
Eventos añadidos en v1.2.8 (segunda tanda — bajo riesgo, sin consumidor en `LocationMap.tsx`):

- `duplicate-threshold-changed` — `{ threshold: number }` (emisor: `DuplicatesList.tsx`; consumidor: `use-duplicate-count.ts`).
