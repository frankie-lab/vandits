...
- `v1.2.20`: versión actual; POI-N v2 materializado. `computePoiMaturity` añade lectura de `customData/custom_data.geo_resolution.status` y aplica **techo** vía `Math.min(ladder, ceilingFromGeoResolutionStatus(status))`: `pending_review`→POI-4, `needs_name_fix`→POI-3, `needs_coord_fix`→POI-2, `geo_irrecoverable`→POI-1 fijo. Sin flag → ladder libre. Tokens `poi.maturity.0..10` recalibrados a la paleta producto-aprobada (gris neutro → gris cálido → amarillo apagado → amarillo → amarillo intenso → ámbar suave → ámbar → verde amarillento → verde suave → verde); sin rojo, sólo namespace `poi.maturity.*`. Helper exportado `ceilingFromGeoResolutionStatus`. Reinyección post-validación humana documentada (`UPDATE locations SET custom_data = custom_data - 'geo_resolution'` → libera techo, sin re-enrich). +10 contract tests en `poi-maturity.test.ts` (40/40 pass). Overlay `MaturityBadgeLayer` consume tokens nuevos sin cambios. NO toca: `createCustomIcon`, `resolvePoiVisualGrammar`, `getPoiCurationLevel`, marker base, colecciones, health rings, collection tints, popup, hero, ratings, export, sharing, datos, RLS, edge functions, migraciones. Doc: `docs/contracts/poi-maturity-visual-contract.md` §2/§4/§5.
