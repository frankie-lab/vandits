...
1.x — Producto funcional
  ...
  1.4.6        PR-IDENTITY-ROOT-DOC-1: contract A/B/C/D doc DRAFT→ACTIVE reflejando runtime ya materializado (Deno SoT + cliente espejo + fixtures + parity test + filtro UI + partición salud)
  1.5.0        PR-IDENTITY-ROOT-PERSIST-1: persistencia A/B/C/D en locations (columna identity_root_status + identity_skip_reason + trigger BEFORE INSERT/UPDATE + backfill 5447 filas + índice parcial). Mirror SQL del clasificador Deno/cliente con paridad de orden de decisión y canon ISO2.
  1.5.1        PR-MAINTAIN-FOOTER-1: Mantener footer primary = Resolver (nunca Exportar); 1 POI abre popup canónico, N POIs abre HealthRepairPreviewDialog; Exportar demoted a menú
  1.5.2        Mantener: oculta fila A/B/C/D del FilterBar y añade universo 'user-action' (solo A+C requieren intervención humana). Footer Resolver pendiente de rediseño (PR siguiente).
  1.5.3        PR-EXPORT-DISCOVERY-1 docs/contracts/poi-export-canon.md
  1.5.4        PR-EXPORT-3 Export Resolver UX
  1.5.5        PR-EXPORT-3 fix · Eliminar typed-token EXPORTAR y threshold local del EffectiveActionFooter; export delega 100% en ExportResolver
  1.5.6        PR-EXPORT-4 — Semántica internal vs public en ExportResolver (copy + contadores scope-aware, sin No incluidos genérico)
  1.5.7        PR-EXPORT-5 — Export Content Model & Popup Parity (layers + KML HTML + CSV/JSON/GeoJSON enrichment)
  1.5.8        PR-EXPORT-6 GuruMaps target renderer
  1.5.9        PR-EXPORT-7: GuruMaps simplification — drop links/hashtags, add collection, expand description
  1.5.10        PR-EXPORT-8: GuruMaps full popup content — sin truncado, ficha completa
  1.5.11        PR-AUTH-BLANK-1 — shared auth state + root redirect fallback
  1.5.12        PR-BOOT-PERF-1: defer batch-enrich storm + boot-gate + getActive concurrency limit
  1.5.13        PR-BOOT-PERF-1 closure: after-PR1 dossier + boot-gate contract test
  1.5.14        PR-BOOT-PERF-2: concurrent catalog pagination (window 3) + contract test
  1.5.15        PR-IMPORT-UX-1: hub de importación unificado (3 vías canónicas: Archivos · Web · OneDrive · fotos) + Biblioteca, audit operability, contract test.
  1.5.16        PR-IMPORT-UX-1 closure: OneDrive sub-tabs split into Importar/Avanzado, DocumentsPanel marked as Biblioteca, import canon docs + memory synchronized, backlog explicit
  1.6.0        PR-IMPORT-UX-2 — hub Contenido rediseñado como wizard guiado (3 cards + 5 pasos)
  1.6.1        PR-IMPORT-UX-3 — Panel "Fuentes de importación": 3 tabs operativas (Archivos · Web · Imágenes), wizard PR-IMPORT-UX-2 retirado, histórico contextual por fuente.
  1.6.2        PR-IMPORT-UX-4: sub-toggle Accion/Historico por tab + CTA real en PanelFooter elevada desde el hijo via hidePrimaryCta + onPrimaryStateChange. Footer canonico no decorativo, sin CTA duplicada inline. Historico con CTA secundaria 'Nueva importacion'. Tooltips en disabled. No tocados parsers/scrapers/edge/schema/RLS.  ← versión actual (stable / current)
...
- `v1.4.1`: **Root Status B → Geo Maintenance scoped** (refs: `docs/audits/root-status-b-geo-maintenance-scoped-plan.md`, `docs/audits/root-status-b-geo-maintenance-scoped-postflight.md`, `docs/contracts/root-status-resolution-contract.md`). Bump **patch** `1.4.0 → 1.4.1`. **Added**: (1) acción **"Abrir en Geo Maintenance"** en el grupo Root Status B (`systemDebt`) de `HealthRepairPreviewDialog`, gated por capabilities `view_geo_maintenance` + `run_geo_backfill`; (2) bridge scoped `lovable:open-geo-maintenance-scoped` (`src/shared/events/geo-maintenance-handoff.ts`) que transporta los IDs B exactos + `mode`/`source`/`reason` desde el triage al panel destino, con `pendingHandoff` para entrega fiable aunque el panel no esté montado al despachar; (3) banner ámbar informativo en `GeographyBackfillPanel` confirmando recepción del handoff y dejando explícito que **no se ha escrito nada todavía**. **Changed**: el grupo Root Status B deja de ser únicamente Exportar / Mapa cuando el usuario tiene la capability adecuada; `GeographyBackfillPanel` puede recibir handoff scoped desde `HealthRepairPreviewDialog` y preselecciona `selectedIds` automáticamente, requiriendo aún clic humano explícito sobre el CTA del panel para lanzar el job. **Safety**: el click en el modal **no ejecuta backfill**; la escritura sólo ocurre tras CTA explícito dentro de `GeographyBackfillPanel`; D repair intacto — `enqueue_health_repair` nunca recibe IDs B; payload del bridge contiene exclusivamente IDs B (regla dura de escritura del contrato A/B/C/D respetada); botón gated por `view_geo_maintenance` + `run_geo_backfill` (sin capabilities, el botón no se renderiza). **Tests**: 34/34 pass — 8 nuevos T1–T8 en `src/test/root-status-b-geo-maintenance-handoff.test.tsx` (gating-off/on, handoff-emit, no-write-on-click, payload-scope-B-only, destination-receives, destination-preview-required, destination-missing, mixed-with-D) + 26 de regresión (`health-repair-dialog`, `health-repair-resolve-button-wiring`, `health-repair-triage-dialog`). **Breaking**: ninguno. NO toca: datos, schema, backend, marker fill, POI-N, health rings, PR-EXPORT-2 core, Nominatim.
 - `v1.6.2`: PR-IMPORT-UX-4: sub-toggle Accion/Historico por tab + CTA real en PanelFooter elevada desde el hijo via hidePrimaryCta + onPrimaryStateChange. Footer canonico no decorativo, sin CTA duplicada inline. Historico con CTA secundaria 'Nueva importacion'. Tooltips en disabled. No tocados parsers/scrapers/edge/schema/RLS.
 - `v1.6.1`: PR-IMPORT-UX-3 — Panel "Fuentes de importación": 3 tabs operativas (Archivos · Web · Imágenes), wizard PR-IMPORT-UX-2 retirado, histórico contextual por fuente.
 - `v1.6.0`: PR-IMPORT-UX-2 — hub Contenido rediseñado como wizard guiado (3 cards + 5 pasos)
 - `v1.5.16`: PR-IMPORT-UX-1 closure: OneDrive sub-tabs split into Importar/Avanzado, DocumentsPanel marked as Biblioteca, import canon docs + memory synchronized, backlog explicit
 - `v1.5.15`: PR-IMPORT-UX-1: hub de importación unificado (3 vías canónicas: Archivos · Web · OneDrive · fotos) + Biblioteca, audit operability, contract test.
 - `v1.5.14`: PR-BOOT-PERF-2: concurrent catalog pagination (window 3) + contract test
 - `v1.5.13`: PR-BOOT-PERF-1 closure: after-PR1 dossier + boot-gate contract test
 - `v1.5.12`: PR-BOOT-PERF-1: defer batch-enrich storm + boot-gate + getActive concurrency limit
 - `v1.5.11`: PR-AUTH-BLANK-1 — shared auth state + root redirect fallback
 - `v1.5.10`: PR-EXPORT-8: GuruMaps full popup content — sin truncado, ficha completa
 - `v1.5.9`: PR-EXPORT-7: GuruMaps simplification — drop links/hashtags, add collection, expand description
 - `v1.5.8`: PR-EXPORT-6 GuruMaps target renderer
 - `v1.5.7`: PR-EXPORT-5 — Export Content Model & Popup Parity (layers + KML HTML + CSV/JSON/GeoJSON enrichment)
 - `v1.5.6`: PR-EXPORT-4 — Semántica internal vs public en ExportResolver (copy + contadores scope-aware, sin No incluidos genérico)
 - `v1.5.5`: PR-EXPORT-3 fix · Eliminar typed-token EXPORTAR y threshold local del EffectiveActionFooter; export delega 100% en ExportResolver
 - `v1.5.4`: PR-EXPORT-3 Export Resolver UX
 - `v1.5.3`: PR-EXPORT-DISCOVERY-1 docs/contracts/poi-export-canon.md
 - `v1.5.2`: Mantener: oculta fila A/B/C/D del FilterBar y añade universo 'user-action' (solo A+C requieren intervención humana). Footer Resolver pendiente de rediseño (PR siguiente).
 - `v1.5.1`: PR-MAINTAIN-FOOTER-1: Mantener footer primary = Resolver (nunca Exportar); 1 POI abre popup canónico, N POIs abre HealthRepairPreviewDialog; Exportar demoted a menú
 - `v1.5.0`: PR-IDENTITY-ROOT-PERSIST-1: persistencia A/B/C/D en locations (columna identity_root_status + identity_skip_reason + trigger BEFORE INSERT/UPDATE + backfill 5447 filas + índice parcial). Mirror SQL del clasificador Deno/cliente con paridad de orden de decisión y canon ISO2.
