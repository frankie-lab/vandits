...
1.x — Producto funcional
  ...
  1.3.0        Canon v3 marker fill — Fase 1: helper SoT `getPoiMaturityColor` (sin tocar renderer)  ← versión actual (stable / current)
...
- `v1.3.0`: versión actual; **Fase 1 canon v3 marker fill** (`docs/contracts/marker-fill-canon-v3.md`). Nuevo helper SoT cromático `src/domains/content/lib/poi-maturity-color.ts` con API `getPoiMaturityColor(loc) → { level, fill }`: `level` proviene de `computePoiMaturity(loc)` (incluye techo por flag `custom_data.geo_resolution.status` v2 ya aplicado); `fill` se lee directamente del token `poi.maturity.<level>` como `hsl(...)`. Función pura, sin lectura de DOM ni dependencia de `getPointVisualState`. 19 contract tests (`src/test/poi-maturity-color.test.ts`): mapeo correcto POI-0..POI-10, los 4 techos `geo_resolution` (`geo_irrecoverable`→POI-1, `needs_coord_fix`→POI-2, `needs_name_fix`→POI-3, `pending_review`→POI-4), sin flag → ladder libre, status desconocido → sin techo, `null`/`undefined` → POI-0. Bump **minor** reservado para la migración del canon cromático. **Fase 1 NO toca renderer**: `createCustomIcon`, `resolvePoiVisualGrammar`, marker base, colecciones, health rings, owner identity, overlay POI-N y popup permanecen idénticos — el mapa NO cambia visualmente. `getPointVisualState` se conserva como semántica legacy (filtros, telemetría, leyendas); `poi.state.*` no se elimina. Fases 2..6 quedan diferidas a PRs futuros.
