...
1.x — Producto funcional
  ...
  1.2.18       Overlay diagnóstico POI-Maturity admin-gated, OFF por defecto  ← versión actual (stable / current)
...
- `v1.2.17`: versión actual; nuevo helper canónico `computePoiMaturity(loc)` (`src/domains/content/lib/poi-maturity.ts`) que devuelve nivel POI-0…POI-10 según contrato visual ya documentado (`docs/contracts/poi-maturity-visual-contract.md`). Función pura, ladder monotónico estricto: POI-0 sin dato útil → POI-1 sólo coords → POI-2 sólo nombre → POI-3 nombre + coords WGS84 válidas → POI-4 raw_geocode → POI-5 país/continente → POI-6 región/zona → POI-7 descripción enriquecida (descarta placeholders evasivos del LLM vía `isUnverifiableDescription`) → POI-8 media validada → POI-9 categoría/tags → POI-10 curado completo (`geoHealth='ok'` + `enrichment_status='enriched'` + `enriched_data.observacion`). Reglas DURAS verificadas por contract tests (`src/test/poi-maturity.test.ts`, 19 casos): coords inválidas (Null Island, fuera de rango, NaN) no pueden superar POI-2; ausencia de `raw_geocode` cierra en POI-3; descripción sin geografía previa NO salta a POI-7; placeholder `(sin país)` no cuenta como país resuelto; POI con narrativa "no se puede generar descripción verificable" se queda en POI-6. Acepta entradas camelCase (`GeoLocation` del dominio) y snake_case (filas Supabase). NO toca renderer del mapa (`LocationMap.tsx`, `createCustomIcon`, `resolvePoiVisualGrammar`, `levelKey` PR-MAP-CANON-3 intactos); NO toca datos, RLS, edge functions ni base de datos; NO re-enrich; sin migraciones SQL.
