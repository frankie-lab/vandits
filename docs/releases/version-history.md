...
- `v1.3.3`: versión actual; **Fase 3.1 canon v3 marker fill** — fix de mapping frontend. Auditoría previa detectó que TODOS los POIs enriched aparecían como POI-3: `dbLocationToGeoLocation` no copiaba `raw_geocode` (ni `geo_resolved_at`/`geo_confidence`/`geo_source`) desde `v_locations_resolved`, así que `computePoiMaturity` no podía pasar de POI-3. Esta versión extiende `GeoLocation` con `rawGeocode?: unknown` + 3 campos geo opcionales y actualiza `dbLocationToGeoLocation` para copiarlos. Nuevo test `src/test/db-transformers-raw-geocode.test.ts` (5 tests) blinda la regla y la integración con `computePoiMaturity`. Cambio aditivo. **Deuda residual separada**: ~340 POIs enriched sin `raw_geocode` en DB requieren backfill server-side (fuera del alcance). NO toca: `computePoiMaturity`, renderer, tokens, colecciones, health rings, datos, RLS, edge functions, migraciones. Fases 4..6 diferidas.
