...
1.x — Producto funcional
  ...
  1.5.0        PR-IDENTITY-ROOT-PERSIST-1: persistencia A/B/C/D en locations (columna identity_root_status + identity_skip_reason + trigger BEFORE INSERT/UPDATE + backfill 5447 filas + índice parcial). Mirror SQL del clasificador Deno/cliente con paridad de orden de decisión y canon ISO2.  ← versión actual (stable / current)
