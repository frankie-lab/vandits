...
| 8. Fix denormalización `zone` texto NULL con `zone_id` FK | Abierto | Backend / vista | ~1.700 POIs (IT 923, ES 493, FR 230, GB 30, US 26) tienen `zone_id` válido pero `zone` texto NULL en `v_locations_resolved`. Bug de vista o de writeback de `locations.zone`. Bloquea promoción a POI-10 sin re-enrich. Diagnóstico: [`docs/audits/territorial-equivalence-global-implementation-audit.md`](./audits/territorial-equivalence-global-implementation-audit.md) §5 deuda #1. |
| 9. Codificar `has_provincia` en `resolveAllFks` | Abierto | Backend / resolver geo | El resolver es agnóstico al país y emite `zone_id` espurio en NL/SE/NO/FI (99 POIs) y futuros MX/BR/AU/JP/CO. Cumplir contrato §3 y §5 de [`docs/contracts/territorial-equivalence-canon.md`](./contracts/territorial-equivalence-canon.md). Requiere mirror canónico TS + Deno y contract tests paritarios. |
