...
| 9. Codificar `has_provincia` en `resolveAllFks` | **T2A-code + T2A-data + T2A-wire aplicados (v1.3.14)** | Backend / resolver geo | **T2A-code (v1.3.13):** mirror técnico transversal del canon en `src/shared/geography/territorial-canon.ts` + espejo Deno con 39 entradas. **T2A-data Portugal piloto (2026-05-21):** 18 distritos re-parenteados + 223 POIs remapeados a CCDR canónica + snapshot en `location_geo_provenance`. **T2A-wire (v1.3.14, DONE):** `resolveAllFks` post-procesa los IDs con `applyCanonToResolvedFks` (descarta `zone_id` si `!hasProvincia`, promueve `admin3_id → locality_id` si `municipioField='locality'`, anula `zone_id` si `region==zone` fuera de whitelist §4); `getLocationHierarchy` omite zone si `!hasProvincia` y colapsa `region==zone` whitelisted; `GeographyTree` aplana el nivel Provincia para países sin provincia (reescribiendo paths descendientes); parsers KML/KMZ/GPX/GeoJSON/CSV pasan por `applyCanonToParsed` en `toKMLDocument` (helper único `src/shared/import/canon-validator.ts`). País ISO2 desconocido = passthrough legacy. Sin recálculo de POIs existentes (matiz §1 obligatorio): el wire SOLO afecta futuros resolves/imports/renders. Tests: `wire-resolver`, `wire-hierarchy`, `wire-imports`, `wire-region-eq-zone`, `wire-geography-tree`. Lint `no-hardcode` ampliado a `GeographyTree.tsx`, `parsers/shared.ts`, `import/canon-validator.ts`. **Pendiente T2.2 (data-otros):** remapeo de catálogo en países distintos a PT (NL/SE/NO/FI/MX/BR/AU/JP/CO con `zone` poblado legacy). Sin backfill, sin re-enrich. |
