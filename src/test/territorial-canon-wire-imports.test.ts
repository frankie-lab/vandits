 ...
 *   - hasProvincia=false ⇒ zone descartado
 *   - municipioField='locality' ⇒ admin3 promovido a locality
 *   - país desconocido = no-op
 *   - idempotente
 */
...
describe('T2A-wire — applyCanonToParsed (parsers)', () => {
  it('SE: descarta zone', () => {
    const p = applyCanonToParsed({ country: 'Sweden', region: 'Stockholm', zone: 'INVALID' }, { emitWarnings: false });
  ...
  it('BR: descarta zone', () => {
    const p = applyCanonToParsed({ country: 'Brazil', zone: 'SP' }, { emitWarnings: false });
    expect(p.zone).toBeNull();
  });

  it('CL: admin3 promueve a locality', () => {
    const p = applyCanonToParsed({ country: 'Chile', admin3: 'Providencia' }, { emitWarnings: false });
  ...
  it('PT (hasProvincia=true, admin3): preserva admin3', () => {
    const p = applyCanonToParsed({ country: 'Portugal', region: 'Norte', zone: 'Braga', admin3: 'Braga' }, { emitWarnings: false });
    expect(p.zone).toBe('Braga');
    expect(p.admin3).toBe('Braga');
  });

  it('país desconocido = no-op', () => {
    const p = applyCanonToParsed({ country: 'Atlantis', zone: 'Z', admin3: 'A' }, { emitWarnings: false });
  ...
  it('idempotente: aplicar dos veces es igual que una', () => {
    const once = applyCanonToParsed({ country: 'Norway', zone: 'X', admin3: null }, { emitWarnings: false });
    const twice = applyCanonToParsed({ ...once }, { emitWarnings: false });
    expect(twice).toEqual(once);
  });

  it('iso2 override prevalece sobre country', () => {
    const p = applyCanonToParsed({ country: 'Spain', zone: 'Madrid' }, { iso2: 'SE', emitWarnings: false });
    expect(p.zone).toBeNull();
  });
});
