  ...
  return (
    <div className={`rounded-lg border transition-colors ${
      ...
      <div className="px-2.5 py-2">
        <div className="flex items-center justify-between gap-2 min-w-0">
          <div className="min-w-0 flex-1">
            <h4 className="font-medium text-xs truncate">{parent.name}</h4>
            {originWp && destWp && (
              <p className="text-[9px] text-muted-foreground truncate">
                {originWp.name} → {destWp.name}
              </p>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-destructive hover:text-destructive" onClick={() => onDeleteRoute(parent.id)}>
              <Trash2 className="w-2.5 h-2.5" />
            </Button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mt-1 text-[9px] text-muted-foreground flex-wrap">
          ...
          <div className="flex items-center gap-0.5">
            {sortedChildren.map((child, i) => {
              ...
              return <Icon key={i} className={`w-2.5 h-2.5 ${color}`} />;
            })}
          </div>
          <Badge variant="outline" className="text-[8px] px-1 py-0 h-3.5">
          ...
          {parent.updatedAt !== parent.createdAt && (
            <span>· Mod: {format(new Date(parent.updatedAt), 'dd/MM/yy')}</span>
          )}
        </div>
      </div>
