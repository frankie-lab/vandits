...
` : '';
  ...
  // nunca qué sistema visual se usa. Sin rama legacy.
  {
    ...
    return `
<div id="${popupId}" data-popup-version="${isPopupGeoCanonicalV1On() ? 'geo-canonical-v1' : 'legacy'}" data-popup-geo-canonical="${isPopupGeoCanonicalV1On() ? 'true' : 'false'}" data-popup-ownership-strip="${(isOwn && isPopupOwnershipStripV1On()) ? 'v1' : 'legacy'}" style="width: ${CARD.maxWidth}px; font-family: ${CARD_FONT_FAMILY}; position: relative; display: flex; flex-direction: column; max-height: ${POPUP_MAX_HEIGHT}; overflow: hidden;"><!-- P-POPUP-15 — Badges debug `P-POPUP-2 ON` / `P-POPUP-3 ON` retirados del render runtime. data-popup-* siguen como hooks de test. -->
${statusBarHtml}
