CREATE OR REPLACE FUNCTION public.enqueue_health_repair(_action text, _scope_mode text, _location_ids uuid[])
...
BEGIN
  ...
  -- PR-B0: el modo del job debe ser uno que respete location_ids sin colisión
  -- de .in() en backfill-admin-fks. 'repair' filtra por su propia RPC y pisa
  -- los IDs explícitos. 'fill' y 'reconcile' los respetan tal cual.
  _job_mode := CASE _action
    WHEN 'partial' THEN 'fill'
    WHEN 'chain'   THEN 'reconcile'
  END;
  ...
  ELSE
    ...
    VALUES (
      _caller, _caller, _job_mode, 'running'::geocoding_job_status,
      _label,
